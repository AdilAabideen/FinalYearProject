...
TERMINAL_SWARM_STATUSES = {"completed", "failed", "canceled"}


def persist_swarm_run_metrics(swarm_run_id: str) -> None:
    db = SessionLocal()
    try:
        swarm_run = swarm_runs_repository.get_swarm_run(db, swarm_run_id)
        if swarm_run is None:
            return

        agent_runs = agent_runs_repository.list_all_runs_for_swarm(db, swarm_run_id=swarm_run_id)
        ...
        metrics = swarm_run_metrics_repository.get_swarm_run_metrics(db, swarm_run_id)
        if metrics is None:
        ...
        metrics.status = swarm_run.status
        metrics.duration_ms = swarm_run.duration_ms
        metrics.agent_run_count = agent_run_count
        metrics.handoff_count = swarm_handoffs_repository.count_swarm_handoffs_for_run(
            db,
            swarm_run_id=swarm_run_id,
        )
        metrics.gate_evaluation_count = swarm_gate_evaluations_repository.count_swarm_gate_evaluations_for_run(
            db,
            swarm_run_id=swarm_run_id,
        )
        ...
        swarm_run_metrics_repository.save_swarm_run_metrics(db, metrics)
    finally:
        db.close()
