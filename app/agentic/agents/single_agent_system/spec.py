...
from .evaluator import SingleAgentAcuityEvaluator
...
def build_single_agent(runtime: AgentRuntime, runtime_config: Optional[RuntimeConfig] = None):
    ...
    try:
        return SSEHandrolledAgent(
            ...
            response_format=SingleAgentOutput,
            runtime_config=runtime_config,
        )
    except Exception as e:
        raise Exception(f"Error building single agent: {e}")
...
def run_single_agent(input: SingleAgentInput, *, verbose: bool = True):
    ...
    try:
        ...
        agent = build_single_agent(runtime)
        ...
        raise Exception(f"Error running single agent: {e}")
